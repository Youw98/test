...
That is enough to run the interface, a synthetic single day and the synthetic
experiment matrix. Those paths need no API key, download or network. The later
daily-data and live-model examples require the credentials stated in their own
sections.
...
the verdict, edit the plan, and record an approval or rejection of the simulated
preview. The browser completes that preview before it records the decision;
approval does not start simulation and is not an actuation command.
...
cannot approve it. A successful generation or re-verification receives a one-use
approval token tied to that exact plan and the current scenario overrides. Every
rendered snapshot also has a one-use decision token so a rejection is bound to the
same fingerprint. Changing the plan or settings makes both tokens stale.
...
Every final browser decision goes to the append-only audit log with the time taken,
plan fingerprint and verification source when it is bound to a current verified
snapshot. The core runner separately records proposals, verdicts, revisions and
fallbacks. Draft browser edits and re-verification attempts are not yet separate
audit events, so R25 is only partially covered. Use `--anonymous` to omit the
operator string from final decision records:

```bash
kasflex ui --anonymous --port 9000 --no-browser
```

> Localhost only, single user, no authentication. Approval tokens are global and
> in memory, so concurrent users can invalidate one another's review and a restart
> loses it. This is a simulation interface on your own machine; do not expose it to
> a network or use it as an operational control surface.
...
The first simulates a constraint-blind request without pre-verification. Physical
storage flows remain bounded, while the separate requested trajectory preserves
the violations for the realised audit. The second has the same planner rejected by
the checker until control falls back to the baseline.
...
It fits a demand model to simulator-generated historical days, predicts tomorrow,
and searches for a lower-cost schedule. The built-in example is offline and
deterministic; it is not trained on metered greenhouse operation.
...
You can pass objects with `date`, `forecast` and `actual` condition series to
`build_history`, including externally sourced weather. Demand targets are still
generated by the selected greenhouse adapter; external weather alone does not make
the resulting history metered operational data.
...
the constraint, the hour, the reported value and the feasible bound. For storage
checks, that value is the unsaturated requested counterfactual; physically applied
...
Four properties make the non-operational data job reliable to leave running:
...
A record with `"actuals_available": false` was provisionally scored against the
forecast because the weather archive had not caught up. It is not an out-of-sample
result. Re-run only after a verified measured-weather source is available; the
current automated archive path is Open-Meteo, not the required KNMI ingestion.
...
### One caveat before interpreting a daily record
...
once by hand and inspect source, units, timestamps and row counts before using a
daily record in research.**
...
## Turn on the GreenLight-Gym2 worker (unvalidated here)
